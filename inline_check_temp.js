  ...
  alert("JS Error: " + msg + " @ " + line);
    ...
    function renderMoonNodes(date = selectedSolarDate()){
      ...
      const localMoonX = leftOrbitRx * Math.cos(moonAngle);
      const localMoonY = leftOrbitRy * Math.sin(moonAngle);
      ...
      const orbitWidth = sideRightX - sideLeftX;
      const phaseT = node.phase;
      const sideMoonX = sideLeftX + orbitWidth * phaseT;
      const sideMoonY = sideBaseY - Math.sin(phaseT * Math.PI * 2) * 34;

      const nodeZone = document.createElementNS(svgNS, 'rect');
      nodeZone.setAttribute('x', sideEarthX - 40);
      nodeZone.setAttribute('y', 112);
      nodeZone.setAttribute('width', 80);
      nodeZone.setAttribute('height', 96);
      nodeZone.setAttribute('rx', 10);
      nodeZone.setAttribute('fill', 'rgba(147,197,253,.16)');
      nodeZone.setAttribute('stroke', '#bfdbfe');
      ...
      if (summary) {
        summary.innerHTML = `<div class="moon-nodes-status${season.active ? ' active' : ''}">${season.active ? (t.moon_nodes_active || 'Eclipse season active') : (t.moon_nodes_inactive || 'Outside eclipse season')}</div>`
          ...
          + `<div>${canEclipse
            ? (isSolarCase
              ? (t.moon_nodes_possible_solar || 'Solar eclipse possible near new moon at a node')
              : (t.moon_nodes_possible_lunar || 'Lunar eclipse possible near full moon at a node'))
            : (t.moon_nodes_not_aligned || 'Usually no eclipse: the Moon passes above or below the line')}</div>`;
      }
    }
